...
vector<SearchCoords*> AStar::GetNextStates(SearchCoords* state, SearchCoords(&buffer)[42][34])
{
	...
	if (state->spelunkerState.ropeCount > 0 && map.NodeIsEmpty(*state) && state->climbing==false && !map.NodeIsUnknown(*state)) // if ropes available, use them
	{
...
{
	if (StateFilter != NULL && !(*StateFilter)(newState))
	{
		return currentBest;
	}
...
vector<unique_ptr<ActionHandlerFactory>> AStar::CompletePath(SearchCoords(&buffer)[42][34], SearchCoords* target)
{
	...
	{
		target->action->SetAgentState(target->spelunkerState); // add spelunker state after current action into it
