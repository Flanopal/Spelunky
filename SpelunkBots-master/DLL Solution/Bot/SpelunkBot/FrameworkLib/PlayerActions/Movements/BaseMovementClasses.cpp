...
bool BaseMVClasses::JumpToSpot::Start()
{
	...
	if (playerX - target.x < 0) { dx = 1; jumpPos += 0.8; }
	else { dx = -1; jumpPos += 0.2; }
	...
	{
		if (bot->IsClimbing() || abs(playerX-jumpPos) < 0.2)
		{
...
void BaseMVClasses::JumpToSpot::MyCallback(bool stopped)
{
	...
	if (jumpPos > 0 && abs(bot->GetPlayerPositionXNode() - jumpPos) < 0.2)
	{
		// bot is on the side of current block - best spot for jump
		lib->playerActions->movements->Jump(3);
		jumpPos = -1;
	}
	...
	if (!terminated && GetState() == ActionState::terminated) // jump failed, agent is hanging
	{
		...
		{
			// bot is stucked - hanging on the side of block -> get on it
			horizontalMove = lib->playerActions->movements->SideMoveAt(target.x);
			horizontalMove->registrCallback(make_unique<function<void(bool)>>(bind(&JumpToSpot::MyCallback, this, placeholders::_1)));
			if (horizontalMove->Start())
				lib->playerActions->movements->Jump(3);
...
bool BaseMVClasses::GetOnClimbing::Start()
{
	...
	cout << ">> GET ON CLIMBING " << coords.x << " y " << coords.y << endl;
	if (bot->IsClimbing()) return true;
	int x = (int)coords.x;
	middleX = x + 0.5;
	targetX = middleX;
	// WHile moving, bot will be at proper posiition
	...
	// prepare look up for catching the climbing
...
void BaseMVClasses::GetOnClimbing::MyCallback(bool stopped)
{
	...
	{
		// bot is in proper possition for catching -> jump (in case this is a rope)
		middleTick++;
		if (middleTick == 2)
	...
	if (middleTick == 15)
	{
		// bot is in the middle for a long time -> propably climbing
		cout << "Finished" << endl;
		Finish();
	...
	{
		Stop(); // action failed - out of proper position
	}
	callParentCallback(stopped);
}
